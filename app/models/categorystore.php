 ...
 */
	 ...
	 */
	public function findCountsForItemCategories($id) {
		$sql__id_set = $this->_db->prepareSet((array) $id);

		$this->_db->query("
			SELECT c.category_id, count(c.category_id) AS `count`
			FROM category c
				INNER JOIN item_category ic ON c.category_id=ic.category_id	AND ic.item_id IN $sql__id_set
			GROUP BY c.category_id
			ORDER BY c.category_id
		");

		return $this->_db->getResultAssoc('category_id', 'count');
	}// /method
	 ...
	 */
	public function findForItem($id) {
		if (is_array($id)) {
			...
			$sql = "
				...
				FROM category c
					INNER JOIN item_category ic ON c.category_id=ic.category_id
				WHERE ic.item_id IN $sql__id_set
				ORDER BY c.name
			";
		} else {
			$binds = array (
		...
		return $this->_db->newRecordset($sql, $binds, array($this, 'convertRowToObject'));
	}// /method
	 ...
	 *
	 * @return  array  Assoc-array of names.
	 */
	public function getNamesForItem($item_id) {
		$binds = array (
			':item_id'  => $item_id ,
		);
		...
		return ($this->_db->hasResult()) ? $this->_db->getColumn('name') : array() ;
	}// /method
	 ...
	 */
	public function lookupName($category_id, $default = '') {
		if (null === $this->_lookup) {
			$this->_lookup = $this->findAll()->toAssoc('id', 'name');
		}
		return (isset($this->_lookup[$category_id])) ? $this->_lookup[$category_id] : $default ;
	}// /method
